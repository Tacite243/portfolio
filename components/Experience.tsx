...
const ExperienceCard: React.FC<{ exp: any; index: number; locale: Locale }> = ({ exp, index, locale }) => {
  ...
  return (
    <div className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group">
      <div className="flex items-center justify-center w-10 h-10 rounded-full border border-slate-800 bg-slate-950 text-emerald-500 shadow shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 z-10 transition-colors group-hover:border-emerald-500/40">
      ...
      >
        <motion.div
          ...
          className="p-6 rounded-2xl bg-slate-900/50 border border-slate-800 hover:border-emerald-500/30 transition-colors duration-300 shadow-xl backdrop-blur-sm cursor-pointer select-none"
        >
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-2" style={{ transform: 'translateZ(20px)' }}>
            ...
            <span className="self-start sm:self-auto text-xs font-mono text-emerald-500 bg-emerald-500/10 px-2 py-1 rounded">
              {exp.period}
            </span>
          </div>
          
          <div className="text-emerald-400 font-medium text-sm mb-4" style={{ transform: 'translateZ(15px)' }}>
            {exp.role}
          </div>
          ...
          {exp.link && (
            <div className="pt-4 mt-2 border-t border-slate-800/60 flex justify-end" style={{ transform: 'translateZ(15px)' }}>
              <a
                href={exp.link}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center space-x-1.5 text-xs text-emerald-400 hover:text-emerald-300 transition-colors"
    ...
    <div className="space-y-12 py-10 w-full overflow-hidden no-scrollbar relative">
      ...
      <div className="w-full px-1 overflow-x-clip py-4">
        <div 
          ...
          className="space-y-12 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-emerald-500 before:via-slate-800 before:to-slate-900"
        >
