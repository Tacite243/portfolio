...
const EducationCard: React.FC<{ edu: any; index: number }> = ({ edu, index }) => {
  ...
  // Valeurs de mouvement pour suivre la position relative de la souris
  const mouseX = useMotionValue(0);
  const mouseY = useMotionValue(0);

  // Transformation des coordonnées de la souris en inclinaison 3D (tilt max de 12 degrés)
  ...
  // Animation d'entrée au scroll (une fois visible)
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="p-8 h-full rounded-2xl bg-slate-900/40 border border-slate-800 hover:border-indigo-500/30 transition-colors duration-300 flex flex-col justify-between shadow-xl backdrop-blur-sm cursor-pointer select-none relative overflow-hidden"
      >
        ...
        <div>
          ...
          {/* Degree & detail */}
          <p className="text-indigo-400 font-semibold mb-1" style={{ transform: 'translateZ(15px)' }}>{edu.degree}</p>
          {edu.detail && (
            <div className="flex items-center space-x-1.5 text-xs text-slate-400 mb-4" style={{ transform: 'translateZ(10px)' }}>
        ...
        {/* Info footer */}
        <div className="flex items-center text-slate-500 text-xs space-x-4 pt-4 border-t border-slate-800/40" style={{ transform: 'translateZ(10px)' }}>
